...
(function () {
  ...
  function loadScript(url, cb) {
    ...
    s.src = url; s.onload = cb || function () { };
    s.onerror = function () { if (cb) cb(new Error('fail ' + url)); };
  ...
  function injectRuffle(isFlashPatch) {
    ...
    loadScript(ORIGIN + '/engines/ruffle/ruffle.js', function (err) {
      if (err) { console.error('Ruffle load failed', err); return; }
  ...
  function injectWaflash() {
    // Find all Flash <object>/<embed> elements
    var objs = Array.prototype.slice.call(document.querySelectorAll(
      'object[type*="shockwave"], embed[type*="shockwave"], object[data$=".swf"], embed[src$=".swf"]'
    ));
    ...
    import(ORIGIN + '/engines/waflash/waflash-player.min.js').then(function (mod) {
      ...
      if (typeof createWaflash !== 'function') { console.error('waflash createWaflash not found'); patchSites(); return; }

      objs.forEach(function (o, idx) {
        var src = o.getAttribute('data') || o.getAttribute('src') || (o.querySelector('param[name=movie]') || {}).value;
        if (!src) return;

        // Create replacement container with canvas + status (waflash needs these)
        var box = el('div', { style: 'position:relative;width:100%;height:auto;min-height:400px;background:#000' });
        var statusId = 'waflashStatus_' + idx;
        var canvasId = 'canvas_' + idx;

        var statusDiv = el('div', { id: statusId, style: 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#9AA4B2;font-size:14px;text-align:center;z-index:5;display:none' });
        var canvas = el('canvas', { id: canvasId, tabindex: '1' });
        canvas.style.width = '100%'; canvas.style.height = '100%';
        canvas.style.background = cfg.backgroundColor || '#000';

        box.appendChild(statusDiv);
        box.appendChild(canvas);
        o.parentNode.replaceChild(box, o);

        // Temporarily set ids that waflash expects
        var oldCanvasId = document.getElementById('canvas');
        var oldStatusId = document.getElementById('waflashStatus');
        canvas.id = 'canvas';
        statusDiv.id = 'waflashStatus';
        ...
        } catch (e) { console.error('waflash create error', e); }

        // Restore ids for next iteration
        canvas.id = canvasId;
        statusDiv.id = statusId;
      });
      patchSites();
    }).catch(function (e) { console.error('waflash module load failed', e); patchSites(); });
  }
  ...
  function patchSites() {
    ...
    try {
      // generic: neutralize common ad/redirect popups, route new windows to our browser
      window.open = (function (orig) {
        return function (u) {
          try { if (A && u) { A.openUrl(u, 'mobile'); return null; } } catch (e) { }
  ...
  if (inj.desktopMode && inj.controls && inj.controls.enabled !== false) {
    loadScript(ORIGIN + '/controls.js', function () {
      if (window.FBControls) {
        window.FBControls.mount(inj.controls, {
          online: true,
          engineName: ({ ruffle: 'Ruffle', waflash: 'Waflash', flashpatch: 'FlashPatch' })[engine] || engine
        });
      }
    });
  }
