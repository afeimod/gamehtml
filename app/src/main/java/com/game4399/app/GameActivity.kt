...
class GameActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 应用屏幕比例（fit 默认，或用户上次选择的 16:9 / 4:3 / 3:2 / 1:1）
        // 初次可能页面还没加载完，applyScreenRatio 内部会用 post 延迟到布局就绪
        applyScreenRatio()
    }
    ...
    private val viewClientCallback = object : GameWebViewClient.Callback {
        ...
        override fun onPageFinished(url: String?) {
            ...
            // 屏幕比例：仅在 Flash 播放器页面（player.html / waflash.html）上重应用
            // 其他普通网页保持浏览器原生布局，不强制裁剪
            if (url != null && isFlashPlayerUrl(url)) {
                applyScreenRatio()
            }
        }
    ...
    private fun showLocalSwfEnginePicker(url: String, title: String) {
        val engines = arrayOf("WAFlash (推荐)", "Ruffle 最新版 (内置中文字体)")
        val values = arrayOf("waflash", "ruffle")
        val currentEngine = if (PrefsManager.flashEngine == "ruffle_mhhf") "ruffle" else PrefsManager.flashEngine
    ...
    private fun setupFloatingMenu() {
        ...
        floatingMenu.setCallbacks(object : FloatingMenuView.Callbacks {
            ...
            override fun onOpenAspectRatio() { showAspectRatioPicker() }
    ...
    private fun toggleFullscreen() {
        ...
        // 沉浸式状态切换后窗口尺寸可能微调,让比例重新居中
        binding.root.post { applyScreenRatio() }
    }
    ...
    private fun showFlashEnginePicker() {
        val sp = androidx.preference.PreferenceManager.getDefaultSharedPreferences(this)
        val engines = arrayOf(
            "Ruffle 最新版 (推荐, AS1/2/3 全面支持, 内置中文字体)",
        ...
        // 兼容旧版 mhhf 设置，自动迁移为 ruffle
        val migratedCurrent = if (current == "ruffle_mhhf") "ruffle" else current
        val checked = values.indexOf(migratedCurrent).coerceAtLeast(0)
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Flash 引擎")
            .setSingleChoiceItems(engines, checked) { dialog, which ->
                if (which == 2) {
    ...
    private fun isFlashPlayerUrl(url: String): Boolean {
        return url.startsWith("file:///android_asset/player.html") ||
            url.startsWith("https://flash.local/player.html") ||
            url.startsWith("file:///android_asset/waflash.html") ||
            url.startsWith("https://flash.local/waflash.html")
    }
    ...
    private fun applyScreenRatio() {
        ...
        binding.root.post {
            val js = "(function(){try{return window.__applyAspectRatio(" +
                "'${mode.replace("'", "\\'")}');}catch(e){return null;}})();"
            webView.evaluateJavascript(js, null)
        }
    }
    ...
    private fun showAspectRatioPicker() {
        ...
        val checked = aspectRatioModes.indexOf(current).coerceAtLeast(0)
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("屏幕比例")
