...
object PrefsManager {
    ...
    // ---- 屏幕比例（Flash 播放区）----
    /**
     * 屏幕比例模式：fit(默认铺满) / 16:9 / 4:3 / 3:2 / 1:1
     * - fit: 播放器铺满 WebView 区域（拉伸/等比缩放由引擎自身决定，原始行为）
     * - 16:9 / 4:3 / 3:2 / 1:1: 在 WebView 内部按该宽高比居中显示播放区，多余区域黑边
     *   切换方向/全屏/页面导航时自动重应用。
     */
    val screenRatio: String get() = sp.getString("screen_ratio", "fit") ?: "fit"
